...
//VIP EXP BOOST - [GreenStage]
				if(pc_get_group_level(sd) >= battle_config.gm_exp_boost){
							base_exp = (unsigned int)(base_exp*battle_config.base_exp_vip_boost/100);
							job_exp = (unsigned int)(job_exp*battle_config.job_exp_vip_boost/100);
						}  
	...
	if(!(type&1) && !map->list[m].flag.nomobloot && !md->state.rebirth && (
		...
		for(i = 0; i < MAX_MOB_DROP; i++) {
			...
			// INCREASE DROP RATE IF PLAYER HAS MINIMUM GM LEVEL
			if(sd){
				if(pc_get_group_level(sd) >= battle_config.gm_drop_boost){
					switch(it->type) {
						case IT_HEALING:
							drop_rate= (int)cap_value((mstatus->mode&MD_BOSS) ? (float)(battle_config.item_rate_heal_boss_vip * drop_rate / 100. ) : (float)(battle_config.item_rate_heal_vip * drop_rate / 100. ) ,0,10000);
							break;
						case IT_USABLE:
						case IT_CASH:	
							drop_rate= (int)cap_value((mstatus->mode&MD_BOSS) ? (float)(battle_config.item_rate_use_boss_vip * drop_rate / 100. ) : (float)(battle_config.item_rate_use_vip * drop_rate / 100. ) ,0,10000);
							break;
						case IT_WEAPON:
						case IT_ARMOR:
						case IT_PETARMOR:
							drop_rate= (int)cap_value((mstatus->mode&MD_BOSS) ? (float)(battle_config.item_rate_equip_boss_vip * drop_rate / 100. ) : (float)(battle_config.item_rate_equip_vip * drop_rate / 100. ) ,0,10000);
							break;
						case IT_CARD:
							drop_rate= (int)cap_value((mstatus->mode&MD_BOSS) ? (float)(battle_config.item_rate_card_boss_vip * drop_rate / 100. ) : (float)(battle_config.item_rate_card_vip * drop_rate / 100. ) ,0,10000);
							break;
						default:
							drop_rate= (int)cap_value((mstatus->mode&MD_BOSS) ? (float)(battle_config.item_rate_common_boss_vip * drop_rate / 100. ) : (float)(battle_config.item_rate_common_vip * drop_rate / 100. ) ,0,10000);
							break;					
					}
				}
			}
			...
			mob->item_drop(md, dlist, ditem, 0,drop_rate, homkillonly);
		}
