...
void log_cash_buy_sub_sql (struct map_session_data* sd,char * type,char * npc_name,struct item* itm , int amount,int price){
	...
	i_data = itemdb_exists(itm->nameid);
	
	if(i_data == NULL)
		return;
	
	// Time to get Acc Name and Ip Info from login table.
	if( SQL_ERROR == Sql_Query(map->mysql_handle,"SELECT `userid` ,`last_ip` FROM `login`  WHERE `account_id` ='%d'",sd->status.account_id)){
	...
	Sql_GetData(map->mysql_handle,  0, &sql_data, NULL); 
	safestrncpy(account_name,sql_data,sizeof(account_name));
	Sql_GetData(map->mysql_handle,  1, &sql_data, NULL); 
	safestrncpy(last_ip,sql_data,sizeof(last_ip));
	
	if( SQL_ERROR == Sql_Query(logs->mysql_handle, LOG_QUERY " INTO `cashitemshop` (`Date`, `Mapname`, `AccountID`, `AccountName`,`CharacterID`,`CharacterIPaddr`,`PosX`,`PosY`,`Type`,`NpcName`,`ItemID`,`ItemName`,`ItemCount`,`ItemSerial`,`ItemSlot1`,`ItemSlot2`,`ItemSlot3`,`ItemSlot4`,`ItemRefiningLevel`,`Cash_Price`,`Cash_View`,`Cash_Before`,`Cash_After`	)"
