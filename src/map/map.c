...
bool map_zone_mf_cache(int m, char *flag, char *params) {
	...
	//MAPFLAG BYPASS SKILL
	else if (!strcmpi(flag,"bypass_noskill")) {
		int skill_id;
		...
		if(!( skill_id = skill_name2id(skill_name)) || !skill_get_unit_id( skill_name2id(skill_name), 0)) {
			;/* we dont mind it, the server will take care of it next. */
		} else {
			sprintf(rflag,"bypass_noskill\t%s",skill_name);
			map_zone_mf_cache_add(m,rflag);
		}
	}
	return false;
