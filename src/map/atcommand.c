...
if(!battle_config.official_rates) {
	if(prev_config.item_rate_mvp          != battle_config.item_rate_mvp
	   ...
	   //VIP DROP BOOST - [GreenStage]
       ||  prev_config.item_rate_common_vip       != battle_config.item_rate_common_vip
	   ||  prev_config.item_rate_common_boss_vip  != battle_config.item_rate_common_boss_vip
	   ||  prev_config.item_rate_card_vip         != battle_config.item_rate_card_vip
	   ||  prev_config.item_rate_card_boss_vip    != battle_config.item_rate_card_boss_vip
	   ||  prev_config.item_rate_equip_vip        != battle_config.item_rate_equip_vip
	   ||  prev_config.item_rate_equip_boss_vip   != battle_config.item_rate_equip_boss_vip
	   ||  prev_config.item_rate_heal_vip         != battle_config.item_rate_heal_vip
	   ||  prev_config.item_rate_heal_boss_vip    != battle_config.item_rate_heal_boss_vip
	   ||  prev_config.item_rate_use_vip          != battle_config.item_rate_use_vip
	   ||  prev_config.item_rate_use_boss_vip     != battle_config.item_rate_use_boss_vip
	   //VIP EXP BOOST - [GreenStage]
       ||  prev_config.base_exp_vip_boost         != battle_config.base_exp_vip_boost
       ||  prev_config.job_exp_vip_boost          != battle_config.job_exp_vip_boost 
	  ) {
...
ACMD_FUNC(reloadscript)
{
	...
	char arg[12];
	clif_broadcast(NULL, "O Servidor esta Recarregando os NPCs...", 200, 16, ALL_CLIENT);
	clif_broadcast(NULL, "Você sentirá lag então fique parado até que termine, obrigado pela compreensão!", 250, 16, ALL_CLIENT);
	...
	script->reload();
	//npc->reload();
	snprintf(arg, sizeof(arg), "%s", message);
	npc->reload(arg);
...
/*=============================================================================
* @afk - Comando de Estado Ausente com efeito do Chapéu de AFK e Auto Mensagem.
*----------------------------------------------------------------------------*/
ACMD_FUNC(afk)
{
		...
		if( sd->state.away == 1 ) {
				...
				clif_displaymessage(fd, "Você não está mais ausente.");
				return true;
}

		if (!message || !*message || sscanf(message, "%255[^\n]", temp_msg) < 1)
		{
		sprintf(atcmd_output, msg_txt(1504));
		clif_displaymessage(sd->fd, atcmd_output);
		return false;
		}
		sd->state.away = 1;
		...
		sprintf(atcmd_output, msg_txt(1503));
		clif_displaymessage(sd->fd, atcmd_output);
...
/** @emoteall ***********************************
 * Cause a emote on all online players.         *
 * if the argument random is passed, cause a    *
 * random emote. [GtoKnu] Thanks for Valaris for*
 * happyhappyjoyjoy                             *
 ************************************************/
//int atcommand_emoteall(const int fd, struct map_session_data* sd, const char* command, const char* message)
ACMD_FUNC(emoteall)
{
        struct map_session_data* pl_sd;
        struct s_mapiterator* iter;
        ...
        for( pl_sd = (TBL_PC*)mapit->first(iter); mapit->exists(iter); pl_sd = (TBL_PC*)mapit->next(iter) )
        {
                if(stristr(message,"rand") != NULL){
                        e=rand()%59;
                }else{
                        e = atoi(message);
                        if ( (e > 59) || (e < 0) ){
                                clif_displaymessage(fd, "Digite um número válido. Entre 0 e 59.");
                                clif_displaymessage(fd, "Se quiser um emoticon aleatório digite rand");
                                return -1;
                        }
                }
...
ACMD_FUNC(rates)
{
  ...
  if(battle_config.official_rates) {
	...
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(1298), // Experience rates: Base %.2fx / Job %.2fx
			(battle_config.base_exp_rate * ((pc_get_group_level(sd)>=battle_config.gm_exp_boost)? (float)(battle_config.base_exp_vip_boost / 100.) : 1) + base_exp_rate_vip / 100.),
			(battle_config.job_exp_rate * ((pc_get_group_level(sd)>=battle_config.gm_exp_boost)? (float)(battle_config.job_exp_vip_boost/ 100.) : 1) + job_exp_rate_vip / 100.));
	clif_displaymessage(fd, buf);
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(1299), // Normal Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
	        battle_config.item_rate_common * ((pc_get_group_level(sd)>=battle_config.gm_drop_boost)? (float)(battle_config.item_rate_common_vip/100.) : 1 )/100.,
			 battle_config.item_rate_heal * ((pc_get_group_level(sd)>=battle_config.gm_drop_boost)? (float)(battle_config.item_rate_heal_vip/100.) : 1 )/100.,
			 battle_config.item_rate_use * ((pc_get_group_level(sd)>=battle_config.gm_drop_boost)? (float)(battle_config.item_rate_use_vip/100.) : 1 )/100.,
			 battle_config.item_rate_equip * ((pc_get_group_level(sd)>=battle_config.gm_drop_boost)? (float)(battle_config.item_rate_equip_vip/100.) : 1 )/100.,
			 battle_config.item_rate_card * ((pc_get_group_level(sd)>=battle_config.gm_drop_boost)? (float)(battle_config.item_rate_card_vip/100.) : 1 )/100.);
	clif_displaymessage(fd, buf);
	/*snprintf(buf, CHAT_SIZE_MAX, msg_txt(1300), // Boss Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
	         battle_config.item_rate_common_boss * ((pc_get_group_level(sd)>=battle_config.gm_drop_boost)? (float)(battle_config.item_rate_common_boss_vip/100.) : 1 )/100.,
			 battle_config.item_rate_heal_boss * ((pc_get_group_level(sd)>=battle_config.gm_drop_boost)? (float)(battle_config.item_rate_heal_boss_vip/100.) : 1 )/100.,
			 battle_config.item_rate_use_boss * ((pc_get_group_level(sd)>=battle_config.gm_drop_boost)? (float)(battle_config.item_rate_use_boss_vip/100.) : 1 )/100.,
			 battle_config.item_rate_equip_boss * ((pc_get_group_level(sd)>=battle_config.gm_drop_boost)? (float)(battle_config.item_rate_equip_boss_vip/100.) : 1 )/100.,
			 battle_config.item_rate_card_boss/100. * ((pc_get_group_level(sd)>=battle_config.gm_drop_boost)? (float)(battle_config.item_rate_card_boss_vip/100.) : 1 )/100.);
	clif_displaymessage(fd, buf);*/
	/*snprintf(buf, CHAT_SIZE_MAX, msg_txt(1301), // Other Drop Rates: MvP %.2fx / Card-Based %.2fx / Treasure %.2fx
