...
const char *npc_parse_mapflag(char *w1, char *w2, char *w3, char *w4, const char *start, const char *buffer, const char *filepath)
{
	...
	//Noskill Bypass - [GreenStage]
	else if (!strcmpi(w3,"bypass_noskill")) {
		int skill_id;
		char skill_name[MAP_ZONE_MAPFLAG_LENGTH];
		size_t len = w4 ? strlen(w4) : 0;
		if(w4)
			memcpy(skill_name, w4, MAP_ZONE_MAPFLAG_LENGTH);
	
		if(!( skill_id = skill_name2id(skill_name) ) ) {
			ShowWarning("npc_parse_mapflag: Unknown skill (%s) for 'bypass_noskill' flag! removing flag from %s in file '%s', line '%d'.\n", skill_name, map->list[m].name, filepath, strline(buffer,start-buffer));
		}
		else{
			int idx = map->list[m].skill_bypass_count;
			RECREATE(map->list[m].skill_bypass_id, int, ++map->list[m].skill_bypass_count);
			map->list[m].skill_bypass_id[idx] = (unsigned short)skill_id;
			map->list[m].flag.bypass_noskill = state;			
		}
		}
...
int npc_reload(char arg[]) {
	...
	if(battle_config.dynamic_mobs && !strcmp(arg, "mobscript")){
	...
	for(nsl = npc->src_files; nsl; nsl = nsl->next) {
		if(strstr(nsl->name, "npc/monstros/") && strcmp(arg, "mobscript"))
			continue;
	...
	ShowNpc("Finalizada leitura de '"CL_WHITE"%d"CL_RESET"' NPCs:"CL_CLL"\n"
			 ...
			 npc_id - npc_new_min, npc_warp, npc_shop, npc_script, (!strcmp(arg, "mobscript")?npc_mob:0), (!strcmp(arg, "mobscript")?npc_cache_mob:0), npc_delay_mob);
